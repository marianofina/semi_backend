...
"""
@router.get("/{noticia_id}", response_model=schemas.NoticiaBase)
def obtener_noticia(noticia_id: int, db: Session = Depends(get_db)):
    db_noticia = crud.obtener_noticia(db, noticia_id)
    if db_noticia is None:
        raise HTTPException(status_code=404, detail="Noticia no encontrada")
    return db_noticia
"""

@router.get("/por-usuario", response_model=list[schemas.NoticiaBase])
